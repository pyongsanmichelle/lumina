...
> 2. **Spring Security 自動生成エンドポイント**: `oauth2Login {}` および `logout {}` DSL が登録するパス（`/oauth2/authorization/{registrationId}`, `/login/oauth2/code/{registrationId}`, `/logout`）。これらは `spring.webflux.base-path` の適用対象外（SecurityWebFilterChainが処理するため）であり、仕様として `/bff` プレフィックスを付与しない。
...
2. **CSR時 (ブラウザサイドでの画面遷移・API実行時)**:
   - `GET /bff/auth/me` の呼び出し、および401検知後の遷移は、**いずれも `${BFF_ORIGIN}`（`runtimeConfig.public.bffOrigin`）** を用いる。
     ...
     window.location.href =
       `${runtimeConfig.public.bffOrigin}/oauth2/authorization/keycloak?redirect_uri=${encodeURIComponent(window.location.pathname)}`
     ```
